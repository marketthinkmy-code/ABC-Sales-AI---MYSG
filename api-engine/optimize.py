...
def decide(r):
    """回传 (action, reason)。门槛来自 config(markets.yaml thresholds,可被 Secret 覆盖)。

    分档规则(按 CPL,单位 MYR):
      CPL < SCALE_CPL(30)              → SCALE     加 +BUDGET_STEP_PCT%(20%)
      SCALE_CPL <= CPL < MID_SCALE_MAX(40) → SCALE_MID 加 +MID_STEP_PCT%(10%)
      MID_SCALE_MAX(40) <= CPL <= KILL_CPL(60) → KEEP  监控,不动作
      CPL > KILL_CPL(60)               → PAUSE     关掉
    (另:CPL < 0.7×目标 且 报名≥5 → DUPLICATE 复制放大;CPA>2000 的关掉由 weekly_cpa 每周判)
    """
    ...
    cpl = r["cpl"]
    if cpl and cpl > C.KILL_CPL:
        return ("PAUSE", f"CPL {cpl:.0f} > {C.KILL_CPL:.0f}")
    if cpl and cpl < 0.7 * C.TARGET_CPL and r["results"] >= 5:
        return ("DUPLICATE", f"极佳 CPL {cpl:.0f}, 复制放大")
    if cpl and cpl < C.SCALE_CPL and r["spend"] >= 3 * C.TARGET_CPL:
        return ("SCALE", f"CPL {cpl:.0f} < {C.SCALE_CPL:.0f}, +{C.BUDGET_STEP_PCT:.0f}%")
    if cpl and cpl < C.MID_SCALE_MAX and r["spend"] >= 3 * C.TARGET_CPL:
        return ("SCALE_MID", f"CPL {cpl:.0f} < {C.MID_SCALE_MAX:.0f}, +{C.MID_STEP_PCT:.0f}%")
    return ("KEEP", f"监控 CPL {cpl and round(cpl)}")
...
def apply(r, action):
    ...
    elif action in ("SCALE", "SCALE_MID"):
        pct = C.BUDGET_STEP_PCT if action == "SCALE" else C.MID_STEP_PCT
        new_budget = min(r["daily_budget"] * (1 + pct / 100), C.MAX_DAILY_BUDGET)
...
def run():
    ...
    print(f"[optimize] {C.MARKET} · {C.AD_ACCOUNT_ID} | {len(rows)} 支 ACTIVE | "
          f"目標CPL<{C.TARGET_CPL:.0f} 關>{C.KILL_CPL:.0f} {C.CURRENCY} | DRY_RUN={C.DRY_RUN}")
    ...
    for r in rows:
        ...
        if not C.DRY_RUN and action in ("PAUSE", "SCALE", "SCALE_MID", "DUPLICATE"):
            apply(r, action)

    avg = total_spend / total_results if total_results else 0
    print("  " + "-" * 78)
    print(f"  合計: 花費 {total_spend:.0f} | 報名 {total_results:.0f} | 平均CPL {avg:.0f} {C.CURRENCY}")
