...
PRICE = float(os.environ.get("PRICE_PER_SALE") or 0)   # 客單價(後端課程售價),用 Secret 設
ROAS_KEEP = float(os.environ.get("ROAS_KEEP") or 2.5)
ROAS_SCALE = float(os.environ.get("ROAS_SCALE") or 3.0)
KILL_CPL = float(getattr(C, "KILL_CPL", 60) or 60)
CPA_KILL = float(getattr(C, "CPA_KILL", 2000) or 2000)   # CPA > 此 → 自動關(MYR)
CUR = getattr(C, "CURRENCY", "MYR")
SHEET_ID = os.environ.get("BUYER_SHEET_ID") or "1cXHt9sycxKlYfv8DjeOESQyAZSvz8PSlCVBDxNMBMvg"
...
<header class="top">
  <div class="eyebrow">AI EMPLOYEE BLUEPRINT · {esc(C.MARKET)} · 廣告買單戰報</div>
  <h1>素材買單戰報 <span class="cpa">CPA Dashboard</span></h1>
  <p class="sub">已付款學員 Sheet · 客單價 <b>RM{money(PRICE)}</b> · CPA 關線 <b>RM{money(CPA_KILL)}</b> · ROAS ≥ {ROAS_KEEP} 保留、≥ {ROAS_SCALE} 加預算 · 每週三 10:00 自動更新</p>
</header>
<section class="kpis">
  <div class="kpi"><div class="k-lbl">總買單</div><div class="k-val">{buyers_total}</div><div class="k-sub">Stripe 已付款</div></div>
  <div class="kpi good"><div class="k-lbl">總營收</div><div class="k-val">RM{money(tot_rev)}</div><div class="k-sub">{buyers_total} × {money(PRICE)}</div></div>
...
# ---------- 5.5 CPA > 上限 → 自動關廣告 ----------
def pause_high_cpa_ads(rows):
    """CPA(花費 ÷ 買單)> CPA_KILL 的素材 → 找到對應 ACTIVE 廣告並 PAUSE。
    DRY_RUN 時只印不動;>10 支停手交人工(保護欄)。"""
    from facebook_business.adobjects.ad import Ad
    targets = [r for r in rows if r.get("cpa") and r["cpa"] > CPA_KILL]
    if not targets:
        print(f"  ✓ 無 CPA>{CPA_KILL:.0f} {CUR} 的素材,不需關。")
        return
    ads = [{"id": a["id"], "name": norm(a.get("name", "")),
            "status": a.get("effective_status")}
           for a in AdAccount(C.ACT_ID).get_ads(
               fields=["id", "name", "effective_status"], params={"limit": 1000})]
    hits = []
    for r in targets:
        lab = norm(r["name"])
        for a in ads:
            if a["status"] == "ACTIVE" and lab and (lab in a["name"] or a["name"] in lab):
                hits.append((r, a))
    if len(hits) > 10:
        print(f"  ⚠️ 保護欄:CPA 破線要關 {len(hits)} 支 (>10),停手不執行,請人工檢查。")
        return
    for r, a in hits:
        print(f"  🔴 CPA {r['cpa']:.0f} > {CPA_KILL:.0f} {CUR} → PAUSE ad {a['id']} ({a['name'][:32]})")
        if not C.DRY_RUN:
            try:
                Ad(a["id"]).api_update(params={"status": "PAUSED"})
            except Exception as e:
                print(f"     ⚠️ 關閉失敗: {e}")
    print("  (DRY_RUN:以上只印,未真的關)" if C.DRY_RUN else "  ⚡ 已關掉 CPA 破線廣告")


def run():
    C.init_api()
    buyers = read_buyers()
    print(f"[weekly] {C.MARKET} · 讀到 {len(buyers)} 筆買單 | CPA 關線 {CPA_KILL:.0f} {CUR} | DRY_RUN={C.DRY_RUN}")
    life_ad, _, _ = meta_by_ad("maximum")
    week_ad, _, _ = meta_by_ad("last_7d")
    life_rows, life_unattr = build_rows(buyers, life_ad)
    # CPA 超過上限 → 自動關(你的護欄:CPA>2000 關)
    pause_high_cpa_ads(life_rows)
    ...
    mk = C.MARKET
    open(os.path.join(OUT_DIR, f"cpa_report_{mk}.html"), "w", encoding="utf-8").write(htmlout)
    open(os.path.join(OUT_DIR, f"cpa_data_{mk}.json"), "w", encoding="utf-8").write(
        json.dumps({"market": mk, "buyers_total": len(buyers), "unattributed": life_unattr,
                    "lifetime": life_rows}, ensure_ascii=False, indent=1))
    print(f"→ 已產生 reports/cpa_report_{mk}.html（{len(life_rows)} 支素材）")
